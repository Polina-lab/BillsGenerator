    ...
    <div id="v-menu-multi-level" class="container">
      ...
      <!--:open="open"-->
        <ul class="menu-container" :style="alignMenu"  >
            <v-menu-item v-for="(item, index) in menu" :key="`${index}`" :item="item" :path="path" :level="level"  :customProps="customProps"></v-menu-item>
        </ul>
  ...
  data() {
    ...
    methods: {
        ...
        setPath(event , level , item){
          let data = Object.assign({}, item ,{level : level} )
          this.$emit('set-path', event, level, data );
        },
