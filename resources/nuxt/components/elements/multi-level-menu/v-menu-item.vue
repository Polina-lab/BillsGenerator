<template>
    ...
    <li :ref="item.properties.vmmlID" @click="dispatchEventOnClickItem($event, item , level)" @mouseover="hover = true" @mouseleave="hover = false" :class="dropDownItemMenu">
        ...
        <ul :vmml-id="item.properties.vmmlID" v-if="item[this.customProps.children].length > 0 " :style="containerItem" :class="dropDownMenu" >
          <li v-if="level > 0" @click="setPath($event,  level , item); clickItem($event, item , level)" :customProps="customProps" >
            <div :vmml-id="item.properties.vmmlID" style="padding: 6px; background-color: #fff; color: #000;">
              <div class="item-menu-margin-left" :style="{ marginLeft : marginLeft }">
                <div class="item-icon-before" :style="cssIconBefore.style" style="width: 25px;" ><img src="/icons/left_arrow.svg"  class="pointer" style="width: 10px; " /></div>
                <div class="item-text">
                  Select this </div>
              </div>
            </div>
          </li>
            <v-menu-item  v-for="(child, index) in item[this.customProps.children]" :key="`${index}`" :level="level + 1" :item="child" :customProps="customProps"></v-menu-item>
        </ul>
    </li>
</template>
...
export default {
    name: 'v-menu-item',
    props: ['path','level' ,'item', 'customProps' ,'open'],
    data() {
        return {
            isClosed: true,
    ...
    methods:{
        ...
        setPath(event, level , item){
           this.$parent.setPath(event ,level, item)
        },
        /*maybe later*/
        closeAll(event , value ){
          this.$parent.closeAll(event , value)
        },
        dispatchEventOnClickItem(event, item , level){
            console.log('dispatchEventOnClickItem');
            console.log(item)
            this.setPath(event , level, item);
            event.stopPropagation()
            this.closeVMenuItems(item.properties.vmmlID)
            if(item[this.customProps.children].length < 1) {
              this.clickItem(event, item)
             // this.closeAll(event , false );
            }
        else {
